...
const Login = () => {
  ...
  const handleLogin = async (event) => {
    ...
    const loginUrl =
      "https://sterlingbank.egain.cloud/system/ws/v12/authentication/user/login"; // replace with your login endpoint
    const loginData = {
      userName: event.target.username.value, // Updated key to 'userName'
    ...
    try {
      ...
      if (response.ok) {
        const token = response.headers.get("x-egain-session");
        localStorage.setItem("egainSession", token);
        console.log("Login successful, session token:", token);
        navigate("/egain/activity");
  ...
  return (
    <div className="bg-white-400 dark:bg-gray-800 h-screen overflow-hidden flex items-center justify-center">
